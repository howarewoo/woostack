...
```text
exact skill target + proposed corpus bytes
  -> private immutable proposed package/corpora snapshot
  -> deterministic validation of that exact snapshot
  -> explicit approval of that snapshot digest when new or changed
  ...
  -> bounded whole-descendant teardown + per-action last receipts
  -> deterministic assertions + fresh payload-only qualitative graders
  -> quiescence, completeness check, and aggregate deltas
  -> escaped static review + terminal summary
  -> conversational handback; no implementation edit
```
...
- An approval-pending corpus is overlaid into a private immutable complete-package snapshot and validated there before approval or target materialization. Invalid proposals are not approvable. Silence, ambiguity, rejection, a changed digest, or failed exact-byte revalidation writes no target corpus bytes and dispatches no model action.
- Invalid `--baseline-ref`, invalid `--baseline-path`, or combined baseline flags: stop without implicit fallback.
- In Git, the merge-base path requires the collection's canonical `resolve-base.sh`. If that script or either Git lookup is unavailable or unprovable, require one explicit baseline flag rather than duplicating, guessing, or recording absence.
- Target absent at a valid merge base records `<merge-base-commit>:absent`. An exact target proved outside Git without an explicit baseline records `non-git:<sha256-package-hash>:absent`. A target present but unreadable/incomplete at the merge base stops rather than treating corruption as absence.
...
- Partial spawn, worker error, timeout, malformed output, duplicate receipt, missing receipt, grader failure, teardown failure, or mismatched model/tier/effort/capabilities between required identities: preserve the workspace, mark the affected case blocked/failed, and refuse a clean aggregate.
...
- **AC2 — New or changed eval corpora require validated digest approval.**
  - happy: for an untracked corpus or one that differs from `HEAD`, the command creates one private immutable complete-package snapshot with the exact proposed corpus/fixture bytes, validates it, presents its canonical inventory and digest, obtains explicit approval of that digest, revalidates the snapshot, then materializes and exact-byte revalidates only the approved target corpus bytes before preparation.
  - error: invalid cases, silence, ambiguity, rejection, changed snapshot/target bytes, or digest mismatch produces no target corpus write and no model run.
...
- **AC4 — Baselines are explicit and reproducible.**
  - happy: exactly one valid explicit ref/path baseline wins; otherwise a provable package at `git merge-base HEAD "$(resolve-base.sh)"` is copied and identified in the manifest.
...
- **AC7 — Behavior grading distinguishes objective and qualitative evidence.**
  - happy: deterministic assertions execute without a model; each explicitly qualitative criterion uses a fresh payload-only grader with only anonymized output, opaque output text, and one boolean rubric, and preserves its host-owned rationale/receipt.
  - error: a grader with tools, workspace, prior context, environment/network/credentials, any capability, a non-empty `capabilities` receipt, failure, or unsupported assertion cannot be converted to pass.
