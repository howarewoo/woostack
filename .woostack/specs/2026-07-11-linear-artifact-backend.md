...
- Settled: the Linear plan preserves dependency DAGs and overnight fault-isolated tracks, but execution remains sequential under the existing policy; ordinals provide display order, while each issue declares one Git parent and multi-dependency increments must prove the remaining dependencies are merged or reachable.
