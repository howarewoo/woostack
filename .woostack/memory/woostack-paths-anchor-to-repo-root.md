...
- `source "$(dirname "${BASH_SOURCE[0]:-$0}")/resolve-root.sh"` and use `$WOOSTACK_ROOT/.woostack/...`.
  Use the `:-$0` fallback, never bare `${BASH_SOURCE[0]}` — empty when sourced from zsh
  (see [[bash-source-empty-under-zsh-source]]).
