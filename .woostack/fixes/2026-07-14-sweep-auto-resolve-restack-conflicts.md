...
- treat descendant code as untrusted and run focused verification only in an isolated sandbox with repository secrets and credentials removed and outbound networking disabled; block before execution when the host cannot guarantee both controls;
...
- [x] **Step 1: Reproduce with a failing contract test**
  ...
  - Assert that restack conflicts enumerate unmerged paths and stages, inspect the replayed commit, resolve semantically, run focused verification only with credential and network isolation, stage by explicit path, and resume with `gt continue` before `gt submit --stack`.
  - Assert that repeated descendant conflicts re-enter the same recovery loop, while unavailable verification isolation, ambiguity, unsafe or unverifiable results, verification failures, continuation failures, and other restack failures block and preserve the worktree.
...
- [x] **Step 2: Apply the minimal fix**
  - Replace the blanket restack-conflict blocker in `skills/woostack-sweep/SKILL.md` with the enumerate stages → inspect replayed intent → reconcile both sides → isolated focused verification → path-scoped stage → `gt continue` loop.
  ...
  - Treat descendant code as untrusted: remove repository secrets and credentials and disable outbound networking for verification, or block before executing project code when those controls are unavailable.
