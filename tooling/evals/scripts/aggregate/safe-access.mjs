...
async function requirePrivateRunRoot(runRoot) {
  ...
  if (process.platform === 'win32') {
    const [resolvedRoot, resolvedTemp] = await Promise.all([realpath(runRoot), realpath(os.tmpdir())]);
    const relative = path.relative(resolvedTemp, resolvedRoot);
    if (relative === '' || relative.startsWith(`..${path.sep}`) || relative === '..' || path.isAbsolute(relative)) {
      throw new Error('run root must be inside the native user temporary directory on Windows');
    }
    return;
  }
...
function windowsUserTempPath(directory) {
...
async function requirePrivatePublicationParent(parent) {
  ...
  const invalidOwnership = process.platform === 'win32'
    ? !windowsUserTempPath(canonical)
    : (listed.mode & 0o077n) !== 0n || listed.uid !== currentUid;
  if (!listed.isDirectory()
...
async function revalidatePublicationParent(parent, directory) {
  ...
  const listed = await lstat(parent, { bigint: true });
  const invalidOwnership = process.platform !== 'win32' && (listed.mode & 0o077n) !== 0n;
  if (!listed.isDirectory()
    || listed.isSymbolicLink()
    || invalidOwnership
...
async function publishCreateNew(outPath, contents, {
  ...
  try {
    ...
    // Keep the inode referenced until commit or rollback so an unlink/recreate race cannot recycle
    // its identity and trick rollback into deleting a replacement pathname.
