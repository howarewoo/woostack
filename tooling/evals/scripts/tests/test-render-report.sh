...
mkdir "$TMP_ROOT/report-output"
if [ "$("$NODE" -p 'process.platform')" != win32 ]; then
  chmod 700 "$TMP_ROOT/report-output"
fi
...
if (process.platform === 'win32') {
  await mkdir(path.join(root, 'evidence', 'symlink-output.txt'));
} else {
...
forged['candidate-only-status'] = structuredClone(candidateOnly);
...
if (process.platform === 'win32') {
  await writeFile(path.join(root, 'aggregate-symlink.json'), 'not a regular aggregate fixture\n');
} else {
