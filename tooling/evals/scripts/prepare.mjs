...
import {
  ...
  parseFrontmatter,
  validatePackage,
} from '../../../skills/using-woostack/scripts/validate-skill-package.mjs';

const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const DEFAULT_CATALOG_ROOT = path.resolve(SCRIPT_DIR, '..', '..', '..', 'skills');
...
async function materializeGitPackage(gitRoot, commit, layout, tempParent) {
  ...
  const tempRoot = await mkdtempAsync(path.join(tempParent, '.prepare-git-baseline-'));
...
function chooseOutRoot(options, gitRoot) {
  ...
  if (!gitRoot) {
    throw new Error('output root is unproven; pass --out-root explicitly');
  }
  ...
  return runGit(gitRoot, ['check-ignore', '-q', '--', '.woostack/tmp/skill-evals/'], { allowCodes: [0, 1] })
    .then((result) => {
      ...
      throw new Error('default output root is not ignored; pass --out-root explicitly');
    })
    .catch((error) => {
      if (error?.message?.includes('pass --out-root explicitly')) throw error;
      throw new Error('output-root ignore proof failed; pass --out-root explicitly', { cause: error });
    });
}
...
async function chooseBaseline(info, options, tempParent) {
  ...
  if (options.baselineRef) {
    ...
    const materialized = await materializeGitPackage(gitRoot, resolved, info.gitLayout, tempParent);
  ...
  if (options.baselinePath) {
    ...
    const tempRoot = await mkdtempAsync(path.join(tempParent, '.prepare-path-baseline-'));
    tempRoots.add(tempRoot);
  ...
  const materialized = await materializeGitPackage(gitRoot, mergeBase, info.gitLayout, tempParent);
  if (!materialized.present) {
    return {
