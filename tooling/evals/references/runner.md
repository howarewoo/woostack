...
and `git check-ignore` proves that exact location ignored. Do not infer a common root, alter
`.woostack/` or ignore rules, or fall back to a system-temporary directory. If that proof is
unavailable or false, stop and require an explicit `--out-root`. Reject the selected output root
when it resolves inside the candidate or an explicit path baseline.
...
The configured maintainer runner then follows the
[comparative-only decision](../SKILL.md#comparative-only-decision), resolves the applicable fields
once, validates the canonical manifest, and freezes the same configuration for both variants. A
missing isolation, baseline, model-identity, deadline, or teardown proof stops before freeze and
dispatch. Candidate-only and advisory execution are not evaluation evidence.
...
Before manifest freeze, require an approved maintainer runner adapter and prove every generic law
here against its deterministic configuration. Ordinary host subagents and same-session context
separation do not satisfy this contract. Comparative dispatch requires enforceably isolated sibling
workspaces, concrete matching model identity, same-wave intact-pair mechanics, supervisor-owned
evidence, and hard teardown. If any requirement or baseline runnability cannot be proved, refuse
dispatch; do not guess host primitives or fall back to direct/advisory execution.
...
The aggregator emits only final `complete` or `blocked` status and rejects candidate-only manifests.
Assertion failures can coexist with `complete`; missing or invalid execution proof cannot.
