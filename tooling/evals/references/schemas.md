...
`expected` is the canonical unique action identity set and contains candidate then baseline for
every case/repetition pair. Candidate-only expected sets are invalid. `pairs` contains exactly those
selected cases and both canonical candidate and baseline workspace paths. Its `gradingPlan` contains
exactly the selected cases' qualitative assertions.
...
empty-capability grader-receipt, and teardown proofs for the pair. Missing proof for either variant
blocks both grades before either boolean is exposed.
...
`executionStatus` is exactly `complete` or `blocked`. Assertion failures may coexist with `complete`.
Missing, duplicate, malformed, failed, timed-out, unknown, or identity-mismatched required evidence
makes the aggregate `blocked`; every comparison, duration, token, precision, and recall metric is
then `unavailable`, while individually proven repetition evidence remains in `cases`.
...
`node skills/using-woostack/scripts/validate-skill-package.mjs ... --json` emits one JSON object with exactly `schemaVersion`, `valid`,
