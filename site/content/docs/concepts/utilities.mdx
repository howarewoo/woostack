...
telemetry) and answer or render it. Most are read-only; `woostack-qa` and `woostack-respond` write
reports. `woostack-respond` also reads production telemetry and may dispatch gated fix preparation,
so run it only in an explicitly authorized operational context—not as an unconditional CI utility.
