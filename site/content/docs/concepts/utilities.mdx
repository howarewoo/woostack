...
explicit approval; comparative reports require enforced host isolation, while advisory hosts are
limited to candidate-only qualitative smoke evidence. It never edits the target skill.
...
| [woostack-eval](/docs/skills/woostack-eval) | Runs approved behavior and trigger corpora as enforced candidate/baseline comparisons, or explicitly approved advisory candidate-only qualitative smoke. | approved tracked corpus files only; evidence and reports are transient; no target skill edit | `/woostack-eval <skill-path> [--behavior\|--triggers\|--all]` |
