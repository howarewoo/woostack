...
| [woostack-fix](/docs/skills/woostack-fix) | `.woostack/fixes/YYYY-MM-DD-<slug>.md` frontmatter | creates the fix file with `status: draft`, marks it `hardened`, commits the hardened plan for review, waits at the approval gate, then either hands off at `approved` or sets `approved`, `executing`, `in-review`, and eventually `done` during execution/closeout |
