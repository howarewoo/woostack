...
(`linear.saveArtifacts: true`). Git and GitHub own implementation and delivery truth. Bootstrap,
...
Build writes plain `project-spec.md` and `execution-plan.md` Markdown files. Its execution plan orders
independently shippable increments, each with at most one PR. Execute processes one increment at a time
in ordinal order within a run; distinct run IDs may execute concurrently only through disjoint branches,
worktrees, and responsibility surfaces. Every build manages one high-level specification and direct
increment contracts in its retained local run manifest.
...
After proof, a canonical local run is allocated or resumed (and when `linear.saveArtifacts: true`, an
...
build
  ...
  → draft/harden execution plan and write `execution-plan.md`
...
fix
  ...
  → after proof, allocate/resume local run store and use the same plain-artifact path
