...
while Plane Fix arrives in a later increment). Git and GitHub own implementation and delivery truth. Bootstrap,
...
one project, direct increment issues, native dependencies; for Plane: one project, `N` parentless direct work items
with `parent = null`, `N-1` strict blocking relations, and project-label discovery/unioning preserving unrelated
labels). Its execution plan orders independently shippable increments, each with at most one PR. Execute processes
one increment at a time in ordinal order within a run; distinct run IDs may execute concurrently only through
disjoint branches, worktrees, and responsibility surfaces. Every build manages one high-level specification and
direct increment contracts in its retained local run manifest.
## Fix
...
After proof, a canonical local run is allocated or resumed (and when `artifacts.provider: "linear"`, an
exact `--project` is reused or one project is created from validated defaults; when `artifacts.provider: "plane"`,
Fix fails closed before provider access as Plane Fix is unsupported in this increment). An exact source
...
fix
  ...
  → after proof, allocate/resume local run store and use the same plain-artifact path (optional Linear mirror only)
