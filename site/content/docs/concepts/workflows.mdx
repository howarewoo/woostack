...
one project, direct increment issues, native dependencies; for Plane: the canonical `[Repo] owner/name` repository project,
one top-level specification work item with `parent = null`, `N` increment child work items parented to the specification
work item, `N-1` strict sibling blocking relations, and project-label discovery/unioning preserving unrelated labels).
Its execution plan orders independently shippable increments, each with at most one PR. Execute processes
...
exact `--project` is reused or one `[Fix] ` project is created from validated defaults; for `artifacts.provider: "plane"`,
the canonical `[Repo] owner/name` repository project is admitted or resolved, one `[Fix] ` top-level specification
work item created, and increment children parented to it). An exact source
