---
title: Workflow maps
description: "Bootstrap, Build, Fix, and Change under local run authority, optional Linear, Plane, or GitHub mirroring, and repository delivery truth."
---
...
`"plane"`, or `"github"`). Git and GitHub own implementation and delivery truth. Bootstrap,
...
Build writes plain `project-spec.md` and `execution-plan.md` Markdown files. When `artifacts.provider` is
configured for mirroring (`"linear"`, `"plane"`, or `"github"`), one bounded cycle mirrors Build artifacts
to the configured provider (for Linear: one project, direct increment issues, native dependencies; for
Plane: the exact configured project, one top-level specification work item `[Build] <goal>`, child
increment work items with `parent = <spec-item-UUID>`, `N-1` strict blocking relations, and project-label
discovery/unioning preserving unrelated labels; for GitHub: one Project with managed README specification,
parentless repository issues, direct Project membership, and $N-1$ native blocked-by dependencies). Its
execution plan
...
After proof, a canonical local run is allocated or resumed (and when `artifacts.provider` is configured for
mirroring, an exact `--project` is reused or one project is created from validated defaults). An exact source
...
## Local run and optional remote fix/build paths
...
build
  ...
  → write `project-spec.md` (drift-check, mirror once, and read back if mirroring)
...
fix
  ...
  → after proof, allocate/resume local run store and use the same plain-artifact path (optional remote mirror)
...
without mutating a mirrored Linear, Plane, or GitHub project. Standalone Plan closes its existing project as canceled
