...
woostack isolates Git writes so several plans can run at the same time without collision. The
mechanism is the **git worktree**: every Markdown build, fix, and implementation increment does
its writes in a private working tree, while the primary checkout stays clean on the base branch.
