...
Sanitized `.woostack/respond/*.md` files are tracked **decision evidence**, not a third knowledge
store. [woostack-dream](/docs/skills/woostack-dream) may use several corroborating reports as
provenance for a recurring pattern, but one incident cannot establish generalized wisdom.
Response reports never enter `MEMORY.md` and are never pruned. Raw provider telemetry lives only
under ignored `respond/evidence/`; dream and recall never read it.
