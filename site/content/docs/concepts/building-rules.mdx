...
draft Ideate/Harden locally → render owner-only `project-spec.md` and approve path/hash/length →
bounded save/read-back/receipt → draft Plan/Harden locally →
render owner-only `execution-plan.md` and approve path/hash/length plus concise mapping →
bounded save/read-back/receipt → execute → review → hand back
```

The only hard gates are deterministic project-spec-file approval and execution-plan-file approval.
During each gated drafting phase there are no intermediate provider cycles; approval occurs before
save, file identity is revalidated with no-follow and deterministic regeneration, and native
content/graph read-back is compared by the shared provider-presentation canonical fingerprint before
the matching receipt can clear the gate.
...
is the detailed authority for the temporary manifest, owner-only gate files, path/hash/length/
