...
> ideate → write spec → harden spec → commit spec PR → **approve spec** → plan → harden plan
> → append plan to spec+plan PR → **execution handoff** → execute (per increment: implement → commit → review → distill)
...
2. **Spec approval** happens when the committed spec is presented in its PR, before any planning begins.
