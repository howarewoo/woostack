...
question and hardening changes in a secure local draft, render owner-only `project-spec.md` or
`execution-plan.md`, and display only its absolute path, byte length, SHA-256, fingerprint version,
run/process/project identity, and the complete concise task/dependency mapping. The responsible
user approves that exact file identity before one bounded save. Native content or graph read-back is
compared by the shared provider-presentation canonical fingerprint before the matching approval
receipt; both the native bytes and receipts are independently read before the gate clears.
