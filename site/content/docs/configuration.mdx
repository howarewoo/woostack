...
omp additionally provides host-level usage-limit fallback beneath these tiers. Credential
rotation uses the provider logins already available to the host, while `retry.fallbackChains`
is configured in omp's own settings outside `.woostack/config.json`. A temporary host fallback
does not change the tier's configured model.
