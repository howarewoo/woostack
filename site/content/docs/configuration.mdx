...
static policy; selected fix/build/standalone-plan persistence proves live Linear capability before
use.
...
Selected fix/build/standalone-plan persistence is one project, one parent plan issue, and one native
child issue per increment. Missing capability blocks only the selected artifact operation at its
retained safe boundary. Without an exact artifact or explicit persistence request, fix/build/plan
make no Linear call. `woostack-change` ignores the `linear` object.
