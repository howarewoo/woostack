...
`linear.projectStatuses.started` names the native project status used when Execute observes any
direct issue matching the configured `linear.issueStates.executing` or `linear.issueStates.inReview`
mapping by stable native identity and category. It resolves to exactly one status in the selected
workspace/team and must have native category `started`. Execute updates only the exact project's
status, with a stable mutation identity and independent read-back; an exact match is an idempotent
no-op. If all direct issues are `Backlog`/`Todo`, synchronization waits until the selected issue
transitions to the resolved executing mapping and reads back before repository mutation.

`linear.issueStates.executing` and `linear.issueStates.inReview` name the native issue states used by
Execute for active and delivered lifecycle evidence. Execute resolves both mappings to exactly one
native state in the selected workspace/team and compares stable ID, name, and category rather than
using a literal native status name as authority. The resolved `inReview` mapping must have native
category `started`. The full independently read delivery checkpoint, not state alone, proves a
...
woostack remains installed only in OMP or another coding harness. The
