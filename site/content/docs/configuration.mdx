---
title: Configuration
description: Non-secret repository policy for woostack workflow engines and optional artifacts.
---
...
commits, pull requests, reviews, and merge truth. Linear may be configured as an optional artifact
provider for exact feature specifications, plans, or fixes.
...
static policy; optional live artifact checks run only when explicitly requested.
...
| --- | --- | --- |
| `linear` | explicit artifact operations only | optional repository, workspace, team, and native-name hints |
...
The example is artifact-free. Add a `linear` object only when repository policy needs optional
provider defaults. JSON has no comments, so each key is explained below.
...
## Optional Linear artifact configuration
...
Authentication stays in the host's MCP/OAuth secret store. No credential, authorization value, or
credential-file path belongs in repository configuration, prompts, logs, generated files, or
artifacts.

The optional `linear` object may provide default repository/workspace/team hints and native names.
It is not required for artifact-free work. When present, every value is non-empty and the complete
object is validated before it is used. Unknown or incomplete configuration blocks only the
requested artifact operation.

Capabilities are resolved from the requested operation. A read requires exact resource lookup and
pagination; a write additionally requires a stable mutation ID and independent read-back. Skills do
not require lifecycle, assignment, relation, or status mutations merely because Linear is
configured.
...
owns identity, untrusted-data handling, and synchronization receipts.
