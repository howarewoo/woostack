---
title: Configuration
description: Non-secret repository policy for woostack workflow engines and Linear plan availability.
---
...
commits, pull requests, reviews, and merge truth. A validated `linear` object enables automatic
availability preflight for fix/build plan persistence.
...
`/woostack-init --migrate-legacy` route.
...
static policy; live Linear capability is proved by the fix/build/plan workflow before persistence.
...
| --- | --- | --- |
| `linear` | [woostack-build](/docs/skills/woostack-build), [woostack-fix](/docs/skills/woostack-fix), [woostack-plan](/docs/skills/woostack-plan), and explicit artifact operations | enables plan-persistence preflight and supplies repository, workspace, team, and native-name hints |
...
The example is artifact-free. Add a `linear` object when fix/build plans should persist whenever
authenticated official MCP capability is available. JSON has no comments, so each key is explained below.
...
or artifacts. Workflows prove availability from authenticated capability without reading the secret.

The optional `linear` object provides repository/workspace/team hints and native names. When present,
every value is non-empty and the complete object is validated before it is used. Fix/build/plan
preflight the project, issue, sub-issue, relation, and independent read-back capabilities needed for
the plan hierarchy. Fix/build additionally preflight the canceled project-status mapping, exact
project update, and read-back capabilities needed for abandonment.

A successful preflight makes persistence part of the fix/build plan deliverable: one project, one
parent plan issue, and one native child issue per increment. Missing policy or unavailable/incomplete
capability leaves automatic planning artifact-free. `woostack-change` ignores the `linear` object.

`linear.projectStatuses.canceled` names the native status used to close an existing fix/build
project after explicit abandonment. The workflow reads the status transition back and never creates a
project solely to cancel it. Handoff, replanning, and blockers are not abandonment and leave the
project open.
