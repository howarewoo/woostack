...
nor authorize later provider access. A validated `linear` object supplies defaults for caller-selected
exact build/plan artifacts or explicitly requested build/plan persistence, and supplies its configured
team for a proved `/woostack-fix` that creates one issue without an exact issue; it cannot authorize a
write.
...
static policy; selected build/standalone-plan persistence and post-proof fix issue binding prove live
Linear capability before use.
...
## Linear plan configuration

Artifact-enabled profiles connect to the official endpoint:
...
The `linear` object provides repository/workspace/team and native-name defaults for selected build/plan
artifact operations. For a proved new fix without an exact issue, its configured team is the creation
target. Default init may populate missing fields from authenticated read-only discovery and independently
validates any local config write; it preserves valid existing values. That narrow setup does not select
artifact mode, read an artifact, require provider write/read-back capability, or authorize later provider
access.

Before a selected mutation, the workflow verifies the canonical repository association, then resolves
the caller-selected workspace/team for an exact resource or the configured team for new-fix creation,
and rejects missing, malformed, ambiguous, foreign, or conflicting values. It then preflights every
project, issue, sub-issue, relation, mutation, and independent read-back capability needed for the
selected hierarchy.

Build and standalone-plan persistence uses one project, one parent plan issue, and one native child
issue per increment. A proved new fix prompt instead binds or creates exactly one compatible issue
after root-cause proof and never creates a project hierarchy. Missing capability blocks the selected
artifact operation at its retained safe boundary. Before proof, fix makes no Linear call.
