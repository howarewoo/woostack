---
title: Configuration
description: Non-secret repository policy and safe Linear defaults for required fix/build records.
---
...
Linear owns current fix/build product scope and execution plans. Git and GitHub own code, branches,
...
uses them for canonical project creation when needed, and a proved fix uses the configured team for
one-issue creation when needed. Policy cannot authorize a write or clear an approval gate.
...
The init template ships four top-level keys: `models`, `review`, `respond`, and `status`. Other
...
| --- | --- | --- |
| `linear` | Automatic read-only setup in [woostack-init](/docs/skills/woostack-init), required build/post-proof fix records, and selected standalone artifacts | stores validated non-secret repository, workspace, team, and native-name defaults; never authorizes a provider write or content approval |
...
Required fix/build and selected artifact-enabled profiles connect to the official endpoint:
...
requires or selects Linear. Build uses them when creating its one canonical project; a proved new
fix without an exact issue uses the configured team as its one-issue creation target. Default init
may populate missing fields from authenticated read-only discovery and independently validates any
local config write. That narrow setup reads no development artifact, probes no write, and grants no
later access.
...
Build uses one project containing its evolving high-level specification, one direct issue per
increment containing complete executor-ready steps, and native issue dependency relations. A proved
new fix binds or creates one issue after root-cause proof and never creates a project hierarchy.
Missing required capability blocks fix/build at the retained safe boundary; selected standalone
artifact failure blocks only that optional operation. Before proof, fix makes no Linear call.
...
build/plan workflow after explicit abandonment. The workflow reads the status transition back and
never creates a project solely to cancel it. New fix issues are preserved; handoff, replanning, and
blockers are not abandonment and leave projects open.
...
| `respond.remediation` | enum | `"prepare-fix"` | `prepare-fix` may hand a verified repository defect to [woostack-fix](/docs/skills/woostack-fix); `report-only` suppresses every fix handoff. |
