...
`linear` object supplies defaults for optional Linear mirroring. Policy cannot authorize a write.
...
when `linear.saveArtifacts: true`. Mirror failure is recorded in the manifest and is nonblocking for
