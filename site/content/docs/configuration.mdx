...
- one Linear project, specification-bearing project updates, and ordered increment issues own
  multi-issue work.

Project updates hold the approved specification, decisions, lifecycle, and progress. Project and
issue resources carry ownership, dependencies, and acceptance state. Git and GitHub independently
carry the code and PR facts.
...
discover the active profile's tools and prove the required project, specification-document, issue,
project-update, issue-comment, native-state, relation, assignee/delegate, identity, and independent
read-back capabilities. A configured server label or successful authentication screen is not
proof. A missing or partial capability blocks development rather than falling through to a local
record or another transport.

The canonical
[Linear MCP development authority](https://github.com/howarewoo/woostack/blob/main/skills/woostack-init/references/artifact-backends.md)
