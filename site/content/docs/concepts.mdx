...
1. **Project-spec approval** accepts the complete exact local project specification. Only then does
   one bounded synchronization save it to Linear; native content read-back is compared by the
   shared provider-presentation canonical fingerprint before the matching receipt.
2. **Execution-plan approval** accepts the complete exact locally keyed issue contracts and
   ...
   native graph and identity read-back is compared by the shared canonical fingerprints before the
   matching receipt and implementation.
