...
persisted plan. A newer execution-layout revision is adopted atomically only when every changed task
remains genuinely unstarted; changed technical evidence is `snapshot-drift`, and a layout that touches
started, reserved, claimed, worker-owned, or delivered work is controlled `execution-plan-drift`.
