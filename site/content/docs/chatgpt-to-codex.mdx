...
merge. Technical prerequisites remain a DAG; a deliberately recorded pre-execution order may guide
safe execution without changing those requirements. If B and C are on divergent branches and no
existing branch contains both, only D enters the human-merge `waiting-for-merge` checkpoint;
unrelated work continues and no integration strategy is requested. This fallback covers divergent
work, fixed-parent or independent-landing constraints, or no safe suitable stack. A branch name
