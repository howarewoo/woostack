...
In a separate Codex session, hand Orchestrate the verified graph or equivalent understandable work
context and decide whether to run it.
...
Act as my Woostack planning partner in this normal ChatGPT chat. Use the connected GitHub app for repository reads and actual authorized write-capable tools, if available, to publish, after I approve the complete plan, one specification parent with executable native child issues. We will separately hand the verified planning context to woostack-orchestrate in Codex; Execute subagents will implement the children and submit their PRs. This chat ends with the issues. Do not invoke Codex, implementation subagents, or installed Woostack skills here. Do not implement source, create source branches/commits/PRs, or merge.
...
Return the verified parent URL, child links, graph, independently runnable children, any join
decisions, complete handback, and setup gaps. When the native graph is verified, this is a convenient
separate coding-host command:
/woostack-orchestrate --issue <verified canonical GitHub parent issue URL>
Then STOP. Do not invoke it. The user supplies the planning handback; Orchestrate resolves executable
tasks and dependencies from the available context, with or without native links, and asks about
material ambiguity. A separately selected Project may provide status-lifecycle context when
explicitly requested; it is not required for this parent handoff. Missing Plan linkage must be
explained before presenting the publication as verified.
Planning/publication authorizes neither implementation in this chat nor merging. Execute later owns
each selected child's implementation, checks, and PR submission under repository policy. In a coding
host, the preparation wrapper or independently invoked public phases ending in Plan produce the same
issue handoff; those are alternatives, not prerequisites for this chat prompt.
...
For Plan publication, a checklist, `Parent: #N`, or `Depends on #N` documents intent but does not
establish a native link; the publication procedure still requires its own native read-back. After
publication, Orchestrate may interpret tracker prose or other understandable context without treating
those links as an admission gate.
...
identities, graph, runnable roots, join decisions, complete handback, and any optional setup gap,
then **STOP**. For a real publication, open a separately configured [Codex
session](/docs/harnesses/codex) and hand it the verified planning context. A canonical parent URL is
a convenient interpretation hint:

```text
/woostack-orchestrate --issue <verified canonical parent URL>
```

Do not substitute the example URL. Orchestrate interprets the handback or other understandable
issue/task context, resolves executable tasks and dependencies, and schedules them through Execute;
it does not implement the specification parent. Execute owns each selected child's implementation,
checks, and PR submission. Neither the planning chat nor Orchestrate has merge authority. Missing
required Plan relationships must be resolved before presenting the publication as verified; later
Orchestrate interpretation does not require native links to be present in tracker prose.
