...
4. Plan the tracker, complete issues, and genuine technical prerequisites.
...
- genuine technical prerequisite task IDs, later bound to real issue URLs/native IDs;
...
Technical prerequisites form an acyclic evidence graph, not an ordinal chain. Sibling membership and checklist order create no dependencies. Root tasks use the approved integration branch; after publication Orchestrate selects a model-chosen single-parent execution forest before allocation. Its selected parents are compatibility ordering, not native relationship evidence; every technical prerequisite must be on the selected execution ancestor path unless admission proves it is already landed in the approved integration base, and useful parallelism is retained. A dependent can stack on a verified existing parent when current containment and readiness are eligible. Only an explicit `merge-checkpoint` fallback for existing divergence or repository constraints pauses that task as `waiting-for-merge`; this is not an ordinary join strategy or a request for an integration plan. Continue unrelated work, preserve both graphs, and do not create an integration branch, combine branches, rewrite edges, or require an automatic merge. Ask for a decision only when material parent ambiguity remains.
Harden issue contracts and the technical graph as well as the specification: check acceptance coverage, scope, actual verification prerequisites, unique positive ordinals and task identities, duplicate/self edges, cycles, and missing/foreign endpoints. The readable index must distinguish executable issues from design/context issues, baseline/example PRs, exclusions, history, external prerequisites, and the tracker itself. Resolve those roles from that tracker and its fetched issues; issue numbers have no portable meaning. A phase label stays inside the issue that declares it and is not another issue or duplicate writer. Native containment, technical prerequisites, compatibility execution ordering, and Git branch ancestry are separate facts.
...
| D | Direct child of P | B and C | Explicit `merge-checkpoint` fallback when no safe selected parent contains B and C; otherwise a verified existing parent |
...
merge. The technical DAG remains visible evidence; the persisted single-parent execution layout adds
only compatibility ordering and retains independent parallelism. If B and C are on divergent branches
and the selected layout has no safe parent, only D enters the human-merge `waiting-for-merge`
checkpoint under its explicit `merge-checkpoint` fallback; unrelated work continues and no integration
strategy is requested. This fallback covers divergent work, fixed-parent or independent-landing
constraints, or no safe suitable stack. A branch name alone does not resolve the join: containment
must be verified. The workflow never creates an automatic integration branch, combines branches,
rewrites either graph to avoid the join, or merges a PR. Equivalent reordered input resumes the
persisted plan. A newer execution-layout revision requires genuinely unstarted changes except for
verified recovery from legacy execution-parent drift; incompatible started work remains controlled
`execution-plan-drift`. Changed technical evidence is `snapshot-drift`.
...
## Risks and effects
Protect default output compatibility and ordering; compare parsed JSON rather than formatting. Unicode
casefold expands some characters; explicitly test Straße/STRASSE. README changes belong to D. No
deployment beyond the existing CLI package; no migration or data write. Each child is one PR. D may
pause for a Git-parent containment decision only under the explicit `merge-checkpoint` fallback.
## Git parents and stop boundary
A/B start at the approved main tip after fresh ancestry admission. C may stack on A's verified
delivered unmerged branch when current readiness is eligible. For D, Orchestrate selects a persisted
single-parent execution layout before allocation; the selected parent may be an existing verified
branch containing B and C when containment is eligible. Only if the approved layout explicitly uses
the `merge-checkpoint` fallback and no safe selected parent exists does D alone enter
`waiting-for-merge` while unrelated work continues. This is not a request for the user's parent or
integration decision. Do not create an integration branch, combine branches,
rewrite the graph, or merge. Publication ends at verified issues; a separate user invocation of
Orchestrate resumes the same plan.
```
