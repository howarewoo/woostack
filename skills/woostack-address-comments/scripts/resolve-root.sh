...
#
# Sourced (not executed) by resolve-outdir.sh to derive a stable per-project
# OUTDIR key from the repository root, regardless of which workspace package is
# the host agent's current directory.
#
# Precedence:
...
#
# Honors an explicit OUTDIR override downstream unchanged; only the default base switches from
# CWD-relative to root-anchored.
