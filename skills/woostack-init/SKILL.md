...
3. Create missing local support paths only:
   ...
   Init does not create, update, validate, repair, or ignore OMP agent definitions. Delegated work
   selects agents already exposed by the active OMP session; see the
   [OMP host adapter](../using-woostack/references/hosts/omp.md) for capability routing and the
   optional safe cleanup path for unchanged definitions generated by older Woostack releases.
...
4. Perform the automatic read-only [Linear setup](#automatic-linear-setup). Its configured,
   ...
   [GitHub discovery](#selected-provider-github-discovery).
5. Validate JSON/schema/path permissions, ignore policy, report roots, and cross-links. Run the
...
- No source edit outside `.woostack/` except the local managed OMP session-naming assets
  (`.omp/extensions/woostack-session-name.ts`, `.omp/settings.json`, and `.omp/.gitignore`); no
  application scaffold. Init never creates or repairs OMP agent definitions.
