---
name: woostack-init
description: Use when initializing, scaffolding, or repairing a Linear-backed `.woostack/` workspace, including the explicit one-way migration of tracked legacy development records — verifies the host's official Linear MCP connection, writes non-secret repository policy and local knowledge/diagnostic stores, then runs the index builder and `woostack-doctor`. For ongoing workspace-health checks and convention repair, use [`woostack-doctor`](../woostack-doctor/SKILL.md).
---
...
- **Brownfield (user-invoked):** the developer runs `/woostack-init` once to
  set up `.woostack/` in an existing project, or later to repair a partial or stale workspace.
- **Legacy migration (user-invoked):** `/woostack-init --migrate-legacy` is the sole routed owner
  of the explicit one-way migration defined in
  [references/legacy-migration.md](references/legacy-migration.md).
- **Greenfield (via woostack-bootstrap):** the bootstrap skill calls `/woostack-init` as a step in
  its scaffolding sequence so every new project starts with a consistent workspace.
...
4. **Create missing pieces from `templates/`.** After the preflight succeeds, create each item
   ...
   Do not create `.woostack/specs/`, `.woostack/plans/`, `.woostack/fixes/`, or
   `.woostack/overnight/`. Existing copies are legacy migration input. Normal init leaves them
   untouched and lets doctor report one blocking finding per active or ambiguous set. Only explicit
   `--migrate-legacy`, after the same preflight, may execute the packaged
   [one-way migration procedure](references/legacy-migration.md).
...
- `--migrate-legacy` — after successful MCP and target-specific preflight, migrate an explicitly
  enumerated legacy record set through
  [`references/legacy-migration.md`](references/legacy-migration.md). Ambiguous, partial,
  incomplete, unknown-outcome, or foreign evidence preserves every local record.
