...
- `source` — the stable spec, plan, fix, Linear project/document/issue URI, or review marker the learning came from (provenance back to the full "why").
...
- **Stale provenance:** Markdown provenance — `[[specs/<basename>]]`, `[[plans/<basename>]]`, `[[fixes/<basename>]]`, or the legacy `.woostack/specs|plans|fixes/<file>.md` path — resolves against the current repo and warns when missing. Linear provenance must be an exact `linear://project|document|issue/<uuid>` URI (UUID case is canonicalized without changing identity; nested path segments are rejected); static doctor delegates shape parsing to the normalized adapter without credentials. Only explicit `--live` authenticates: the doctor controller performs preflight once, and provenance resolution consumes its temporary non-secret receipt while checking existence, repository/project ownership, managed metadata/schema, and native relation agreement. Failure is an error and never falls back to local files. Review markers (`pr-165`, `address-comments`) are not treated as artifact paths.
