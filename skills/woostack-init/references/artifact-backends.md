...
name, and category rather than literal status names, and require the resolved `inReview` mapping to
have native category `started`. Resolve `projectStatuses.started` to exactly one native project status
and require its native category to be `started`; missing, invalid, ambiguous, foreign, drifted, or
