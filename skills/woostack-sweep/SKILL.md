...
4. **Restack this stack only** — run `gt restack` scoped to the **current** stack so the PRs above
   ...
   - Run the smallest existing focused verification that covers every affected behavior. If it
     passes, stage only the resolved conflict paths with `git add -- <paths>`, then run
     `gt continue`. Repeat the inspect → reconcile → verify → path-scoped stage → continue cycle
     whenever a later descendant commit conflicts. Keep unrelated worktree changes unstaged;
     never `git add -A` or `gt continue -a`.
   - The full restack, including every conflict continuation, must finish successfully before
     ...
     intent is ambiguous or unsafe, no focused verification can establish the combined behavior,
     verification fails, `gt continue` fails, or `gt restack` fails for another reason, stop as a
     blocker and preserve the paused worktree (see Blocker & terminal state).
...
ambiguous or unsafe or cannot be verified safely, focused verification fails, `gt continue` fails,
another `gt restack` failure occurs, or an `address-comments` step would touch the
never-auto-approve set (destructive / secret / auth / network / ambiguous). For a restack blocker,
report the unresolved paths and failed command, and leave the paused worktree and rebase state intact.
The occurrence of a conflict alone is not a blocker. Safety is never relaxed for autonomy.
...
- **Resolve expected restack conflicts.** Inspect every unmerged stage and replayed commit,
  preserve both PRs' intent, run focused verification, stage only explicit conflict paths, and
  continue with `gt continue`; repeat until the stack-scoped restack completes.
- **Stop only when conflict intent is ambiguous or unsafe.** An unverifiable or failing
  resolution, failed continuation, or other restack failure is a blocker; report the evidence and
  preserve the paused worktree and rebase state.
