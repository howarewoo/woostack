...
report = {
    ...
    "remediation_contracts": remediation_contracts,
...
if report.count("### checkout-currency-null — proposed-fix-contract") != 1:
    raise SystemExit("verified root cause does not have exactly one remediation contract")
...
assert_contains "$skill_text" 'RESPOND OWNS NO REPOSITORY MUTATION' 'respond routes fixes instead of editing'
assert_contains "$skill_text" 'binds or creates one canonical Linear issue after' 'repository handoff requires canonical post-proof issue'
...
assert_contains "$skill_text" 'approve-to-execute gate' 'woostack-fix owns the approval gate'
...
assert_not_contains "$skill_text" 'woostack-defer(' 'all response increments are integrated'
finish
