...
for phrase in 'four top-level keys' 'all four' 'eight top-level settings' '"respond": {}' 'respond.provider' 'respond.environment' 'respond.window' 'respond.max_groups' 'respond.remediation' '5 minutes through 30 days' 'from 1 through 5' 'prepare-fix' 'report-only' 'Explicit command arguments' 'Provider credentials' 'authenticated host integration'; do assert_contains "$config" "$phrase" "configuration documents $phrase"; done
