...
printf '{"angle":"bugs","chunk":null,"runner":"r","model":"m","tier":"standard","authority":"advisory-only"}\n' > "$OUTDIR/receipt.bugs.json"
# security + database missing.
...
assert_contains "$out" "database" "lists missing database"
...
printf '{"angle":"bugs","chunk":"chunk-0","runner":"r","model":"m","tier":"standard","authority":"advisory-only"}\n' > "$OUTDIR/receipt.bugs.chunk-0.json"
