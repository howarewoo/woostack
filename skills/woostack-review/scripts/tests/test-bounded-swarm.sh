...
mkdir -p "$work/out"
printf '%s\n' bugs security api architecture docs skills > "$work/out/angles.txt"
...
case "$WOO_REVIEW_ANGLE" in
  api)
    count_file="$OUTDIR/state/api-count"
    count=0
    ...
    printf '%s\n' "$count" > "$count_file"
    if [ "$count" -eq 1 ]; then
      rm -f "$OUTDIR/findings.api.json"
    else
      printf '[]\n' > "$OUTDIR/findings.api.json"
    fi
    ;;
  docs)
...
assert_eq "$(jq -r '.retry_angles | index("api") != null' "$work/out/swarm-metrics.json")" "true" "metrics record retried missing angle"
...
for angle in bugs security api architecture docs skills; do
