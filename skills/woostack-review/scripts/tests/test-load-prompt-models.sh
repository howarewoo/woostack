...
# Array leaf: entry-0 model and effort resolve (provider-scoped).
outdir="$(mktemp -d)"; github_output="$outdir/github_output"; touch "$github_output"
printf '%s\n' '{"models":{"openai":{"standard":[{"model":"gpt-5.5","effort":"low"},{"model":"gpt-5.5-fallback"}]}}}' > "$outdir/config.json"
run_load_prompt "$outdir" "$github_output"
run_model="$(grep '^run_model=' "$github_output" | cut -d= -f2 || echo "")"
run_effort="$(grep '^run_effort=' "$github_output" | cut -d= -f2 || echo "")"
assert_eq "$run_model" "gpt-5.5" "array leaf: entry-0 model resolves"
assert_eq "$run_effort" "low" "array leaf: entry-0 effort wins over tier default"
rm -rf "$outdir"

# Array leaf, flat (provider-less) form: entry-0 effort resolves.
outdir="$(mktemp -d)"; github_output="$outdir/github_output"; touch "$github_output"
printf '%s\n' '{"models":{"standard":[{"model":"gpt-5.5","effort":"high"}]}}' > "$outdir/config.json"
run_load_prompt "$outdir" "$github_output"
run_effort="$(grep '^run_effort=' "$github_output" | cut -d= -f2 || echo "")"
