...
- Runs on `claude-opus-4-8` (every tier → opus). The angle's `tier:` frontmatter now selects **effort**, not model: `standard` → `effort: medium` (`bugs`/`security`/`acceptance`/`architecture`/`design`/`database`/`tests`/`api`/`infra`/`observability`/`simplify`/`production-readiness`), `fast` → `effort: low` (`seo`/`aeo`/`i18n`/`docs`/`deps`/`comments`). The spawning Task call MUST pass `model:` explicitly, plus `effort:` when the Task API accepts it — see Model Routing section above.
