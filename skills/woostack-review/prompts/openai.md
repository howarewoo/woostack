...
**Per-repo override:** resolve using the active run tier: if `$OUTDIR/config.json` has `models.openai.<run_tier>` set, use it; otherwise fall back to flat `models.<run_tier>` (precedence: `FORCE_TIER` > `inputs.model` > `models.openai.<run_tier>` > `models.<run_tier>` > default `gpt-5.5`). The loader normalizes a scalar leaf to `{model, effort?}` and preserves a fallback leaf as an ordered array; select entry 0 before reading `.model`, e.g. when `run_tier=deep`: `jq -r '((.models.openai.deep // .models.deep) | if type=="array" then .[0] else . end | if type=="object" then .model else . end) // empty' $OUTDIR/config.json`.
