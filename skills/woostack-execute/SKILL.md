...
description: Execute one approved Linear project increment, one exact Linear issue, or an approved local run manifest as a resumable sequential Graphite PR workflow. Requires matching approval records; never reviews or merges.
---
...
operations. The `--project` form is the resumable handoff from Build/Fix; `--run` is the resumable
handoff from an approved local run manifest. Both are sufficient without chat memory, a deleted gate
manifest, or fuzzy plan discovery.

## Admission

### Provider admission: one exact resource and two matching records
...
direct-issue graph, native dependency relations, both responsible-user approval receipts, and
canonical repository association. In issue mode (`--issue`), require one exact direct issue and the
same complete reads for its owning project. Never infer either resource or its records from chat
memory, local files, titles, branch names, recent activity, or prior transcript.
...
### Local run admission: exact manifest and gate-file receipts
...
[`run-scoped gated draft manifest`](../woostack-init/references/artifact-backends.md#run-scoped-gated-draft-manifest):
...
2. Require `manifest.json`, `project-spec.md`, `execution-plan.md`, and `.lock` to satisfy the shared
   owner-only `0600` regular-file and ancestor checks; reject symlinks, non-regular files, broader
   permissions, foreign ownership, unexpected entries, and path escape.
3. Validate `manifestVersion`, `manifestRevision`, `workflow`, `runId` matching `<exact-run-id>`,
   `repoRoot` matching the current canonical repository root, `status`, `gate`, empty
   `draft.unresolvedQuestions`, the complete stable-task/dependency graph, and `taskExecutions`.
   Reject `status: "abandoned"`; `status: "completed"` is an independently verified no-work result.
4. Require exactly one local `projectSpecApprovalRecord` and one local
   `executionPlanApprovalRecord` with the shared fields
   `{ runId, gate, manifestRevision, sha256, byteLength, approvedBy, host, approvedAt,
   approvalEventId }`. Require the execution-plan record's immutable approved stable-task mapping
   and dependency snapshots in `displayedApprovalIdentity` to match the admitted plan.
5. Recompute the raw UTF-8 SHA-256 and byte length of each gate file opened no-follow; require exact
   equality with its matching local approval record and require the receipt's manifest revision to
   identify the revision that produced those bytes. Missing, stale, tampered, ambiguous, or
   unreceipted gate files block before repository mutation.

### Recheck semantics

When `--recheck` is provided with `--run`, invoke bounded [`woostack-harden`](../woostack-harden/SKILL.md)
against the current trunk / integration parent tip before execution:

- If rendered `project-spec.md` and `execution-plan.md` remain byte-identical, preserve both local
  approval records and continue.
- If the project specification changes the `project-spec.md` bytes, invalidate both approval records
  and return to gate 1 specification hardening and approval.
- If only the plan/task graph changes the `execution-plan.md` bytes, invalidate only
  `executionPlanApprovalRecord` and return to gate 2 plan hardening and approval.

Without `--recheck`, a compatible parent advance keeps existing approvals and applies normal repository
ancestry and collision reads under the shared repository advancement contract.
...
8. Persist the complete delivery checkpoint:
   ...
   - In local run mode, CAS-update `taskExecutions[stableTaskKey]` from `active` to `delivered` only
     ...
     `manifestRevision`, reopen the manifest and gate files no-follow, and verify every persisted
...
9. Re-read the project/manifest and records, then continue with the next lowest unfinished ordinal
   unless a verified stop marker is encountered. A stop marker pauses before the next task and leaves
   the project/run open with exact resume evidence.
...
repository advancement contract admits that evidence. Rediscover existing commits or PRs before
...
issue, immutable contract fingerprint, allowed paths, worktree path, canonical parent branch/current
admitted tip, retained start/head when resuming, Graphite parent, acceptance, and explicit
prohibitions on scope changes, credentials, source-control boundaries, provider writes, and work in
another worktree.
...
progression. Canonical state alone never proves delivery. Do not change the approved specification,
task graph, assignment, or ownership.
