...
`linear.issueStates.inReview` to exactly one native issue state each. If any direct issue matches
either resolved mapping by stable native identity and category, require the resolved `inReview`
mapping to have native category `started`, resolve `linear.projectStatuses.started`, require its
native category `started`, and make the exact project started (or record an exact started no-op)
before continuing. Do not use a literal native status name as authority. If all direct issues are
...
7. Persist the complete delivery checkpoint, resolve and independently read back the configured
   `linear.issueStates.inReview` mapping, and transition the issue from the resolved executing
   mapping to it. If executing and inReview resolve to the same native status, the transition is an
   independently read idempotent no-op, not a skipped checkpoint. The checkpoint must distinguish
   active from delivered work and is mandatory before teardown, resume, or sibling progression.
   Remove the worktree only after all required evidence is present and the exact worktree is clean.
...
evidence. Verify and validate the one issue, submit and read back one PR, persist the complete
delivery checkpoint, and move/read back the configured `linear.issueStates.inReview` mapping. If
executing and inReview resolve to one native status, record the independently read idempotent
no-op while retaining the checkpoint as the delivered predicate. Remove the clean worktree only
after that checkpoint and all other required evidence are present. Issue mode never advances
...
full delivery checkpoint; canonical state alone never proves delivery or permits teardown, resume,
or sibling progression. Do not change the approved specification, issue graph, assignment, or
ownership.
