...
admit → read exact project + complete direct-issue set
  ...
  → persist full delivery checkpoint + read back inReview mapping/no-op
...
the resolved `inReview` mapping to have native category `started`. If any direct issue matches either
resolved mapping, resolve `projectStatuses.started`, require native category `started`, and
...
issuing a second mutation. Persist issue branch/worktree/run evidence and the full delivery
checkpoint after the relevant boundary. Apply the shared project status gate before repository
mutation and independently read back its separate project-status receipt. Do not alter approved
content, dependency edges, assignment, ownership, or acceptance.
...
the complete delivery checkpoint. Independently resolve and read back the configured inReview
mapping, or record its idempotent no-op when it is the same native status as executing. The
checkpoint is the finished predicate: missing or partial checkpoint evidence blocks teardown,
resume, and sibling progression even when the native issue status already matches inReview.
