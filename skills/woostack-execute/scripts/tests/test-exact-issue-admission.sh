...
checks = [
    ...
    (skill, r"Fix-origin input has exactly one form.*fixApprovalRecord.*exact `--issue`", "fix exact issue admission missing"),
    ...
    (controller, r"Standalone execution remains artifact-optional only when no fix-origin context is present", "standalone execution lost artifact-optional behavior"),
