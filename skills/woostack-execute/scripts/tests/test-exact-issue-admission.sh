...
checks = [
    ...
    (skill, r"issueStates\.executing.*issueStates\.inReview.*stable native identity.*inReview.*started", "configured issue-state resolution missing"),
    (skill, r"complete delivery checkpoint.*finished predicate|checkpoint.*mandatory before teardown.*resume.*sibling", "checkpoint-gated delivery missing"),
    (controller, r"full delivery checkpoint.*idempotent no-op.*teardown.*resume.*sibling", "controller checkpoint/idempotency missing"),
    (artifact, r"issueStates\.executing.*issueStates\.inReview.*stable native identity.*inReview.*started", "artifact issue-state resolution missing"),
...
for case_id in required_cases:
    ...
    if case_id in {"delivery-same-state-is-idempotent", "missing-delivery-checkpoint-blocks-resume"} and "checkpoint" not in prompt.lower():
