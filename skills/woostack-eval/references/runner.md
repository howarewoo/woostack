...
invent host primitives. Comparative dispatch requires provable sibling contexts, same-wave
intact-pair mechanics, baseline runnability, and every isolation boundary below. If any cannot be
proved, the only fallback is the command contract's explicitly user-accepted candidate-only
qualitative smoke branch.
...
freezing `advisory`; rejection or silence stops dispatch. An advisory manifest is always
candidate-only and degraded; aggregation rejects an advisory manifest containing a comparative
pair.
...
unavailable enforcement is captured by advisory assurance and restricts the run to candidate-only
smoke evidence.
