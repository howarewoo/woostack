...
ambient-authority, capability-revocation, descendant-teardown, and grader boundaries. `advisory` is
valid only after the user approves the disclosed unavailable controls, and its manifest must be
candidate-only. Aggregation rejects paired or comparative advisory manifests.
...
`expected` is the canonical unique action identity set. Enforced comparative runs contain candidate
then baseline for every pair. An explicitly accepted candidate-only run contains only candidate
actions for behavior cases with qualitative assertions; trigger and objective-only cases are absent.
Every advisory run uses this candidate-only form. `pairs` contains exactly those selected cases and
retains both canonical candidate and baseline workspace paths. Its `gradingPlan` contains exactly
the selected cases' qualitative assertions.
...
`advisory` always accompanies a `degraded` candidate-only aggregate; a complete advisory comparison
is invalid. Advisory output is candidate behavior smoke evidence, not credential-isolation, network-
denial, process-containment, or other host-enforced security evidence.
