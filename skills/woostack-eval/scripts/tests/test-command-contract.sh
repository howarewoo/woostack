...
const clauses = [
  ...
  ['Preparation and dispatch', 'advisory isolation gate', 'When any workspace, ambient-authority, capability-revocation, descendant-teardown, or grader boundary is unavailable, disclose each unavailable control and require explicit user approval before recording `advisory`; rejection or silence stops dispatch.'],
  ['Preparation and dispatch', 'advisory evidence limit', 'An `advisory` manifest is always candidate-only. It contains only the explicitly accepted qualitative smoke actions described above, and aggregation rejects paired or comparative advisory manifests.'],
