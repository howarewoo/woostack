...
const clauses = [
  ...
  ['Preparation and dispatch', 'scoped capabilities', 'Grant each worker only its approved subset of `read-workspace`, `write-workspace`, and `shell-workspace`; never grant evidence-root access, network, credentials, environment inspection, provider access, the source target, the paired workspace, or unrelated repository content.'],
  ...
  ['Preparation and dispatch', 'last-action receipts', 'After each worker returns, fails, or reaches its deadline, the host supervisor revokes its capabilities and completes the bounded whole-descendant graceful-then-forced teardown before committing evidence. It then commits output first and exactly one create-new action receipt as the final evidence action; missing, duplicate, precommitted, or mismatched receipts block aggregation.'],
  ['Preparation and dispatch', 'isolated blind grading', 'For each planned qualitative assertion, dispatch a fresh grader context with only the schema-defined payload: the opaque output bytes, one anonymized output ID, and one frozen boolean rubric.'],
...
const semanticRequirements = [
  ...
  {
    label: 'bounded action deadline and teardown',
    check: (source) => hasAll(source, [
      ...
      /(?:revoke|close)[^.]{0,100}capabilit/i,
      /refus(?:e|al)[^.]{0,100}dispatch[^.]{0,160}(?:guarantee|deadline|teardown)/i,
      /(?:timeout receipt|receipt)[^.]{0,180}(?:only after|until)[^.]{0,100}(?:all )?descendants?[^.]{0,80}(?:gone|terminat)/i,
    ]),
    fixture: 'Assign a finite positive deadline before manifest freeze. Apply whole-descendant graceful termination, then force-kill. Revoke capabilities. Refuse dispatch without that teardown guarantee. Commit a timeout receipt only after all descendants are gone.',
  },
  {
    label: 'payload-only isolated grader',
    check: (source) => hasAll(source, [
      /grader[^.]{0,260}(?:payload only|payload-only|only (?:the )?(?:schema-defined |opaque )?(?:output|payload))/i,
      /grader.{0,520}(?:no|without)[^.]{0,160}tools?[^.]{0,160}workspace/i,
      /grader.{0,520}(?:receipt[^.]{0,180})?capabilities[^.]{0,100}(?:exactly|must (?:contain|be)|is)[^.]{0,30}\[\]/i,
    ]),
    fixture: 'The grader receives only the schema-defined payload, with no tools or workspace. Every grader receipt capabilities value is exactly [].',
  },
];
