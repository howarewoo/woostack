...
function validateExecutionConsistency(value) {
  ...
  const baselineModes = new Set(value.cases.map((entry) =>
    entry.baseline.length === 0 ? 'candidate-only'
      : entry.baseline.length === value.runs ? 'paired' : 'partial'));
  if (baselineModes.size !== 1 || baselineModes.has('partial')
    || value.cases.some((entry) => entry.candidate.length !== value.runs)) {
    schemaError('/cases');
  }
...
.status-degraded { color: #633c00; background: #fff1cc; }
.status-blocked { color: #6b1515; background: #fde7e7; }
.notice { border-left: .35rem solid #633c00; background: #fff1cc; color: #633c00; padding: .75rem; font-weight: 600; }
