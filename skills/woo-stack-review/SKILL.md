...
4. **Verdict gate** — *default*: present all recommendations as one batched table (thread → finding → recommended verdict → reasoning); the user approves all or overrides specific threads. The **final** verdict per thread is the override where given, else the recommendation. *Skipped under `--auto`* (final = recommended). With no `--auto` on a non-interactive host where confirmation can't be obtained, **abort** with guidance to re-run with `--auto` — never act unapproved.
