...
This is a workflow contract for official Linear MCP capabilities, not a local adapter or transport.
The canonical
...
The specification is project-update content. Never look for or create a Linear document or local
spec/plan record.
...
- exactly one Git parent declaration: the frozen project base for a root, or one dependency issue
  for a stacked increment.
...
1. Discover the complete project issue set and match only the managed identity tuple. Titles never
   match an issue.
...
4. Materialize the desired native dependency/blocker relations, then independently read every
   issue and relation back. Relationship mutation responses are not receipts.
5. Re-read the complete project, current phase chain, issue set, comments, work owners, native
   states, and branch/PR attribution. Compare that independent snapshot with the whole desired
   graph, not only changed issues.
...
evidence cannot be rewritten as unstarted. Any requested scope removal or evidence conflict returns
to the build owner for an explicit project decision or abandonment.
