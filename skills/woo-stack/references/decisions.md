...
3. **Capabilities are opt-in.** For billing, transactional email, webhooks, chat, observability, realtime, and feature flags, first ask whether the project needs the capability at all. If no, skip its package and env entirely. If yes, confirm the provider.
