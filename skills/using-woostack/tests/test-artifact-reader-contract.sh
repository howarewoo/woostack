...
for name in skills:
    ...
    if not re.search(r"optional.{0,120}(?:Linear |Plane |GitHub |Linear or Plane |Linear, Plane, or GitHub )?artifact|artifact.{0,120}optional|needs no (?:Linear|Plane|GitHub|provider) read", folded, re.I):
    ...
    if "artifact-providers/github.md" not in text:
        failures.append(f"{name}: does not link GitHub provider profile")
    if re.search(r"(?:requires?|must have|exactly one) (?:an? |the )?(?:managed |Linear |Plane |GitHub )?(?:issue|project|work item)", folded, re.I):
