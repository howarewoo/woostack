...
user's current goal. For `woostack-build` and `woostack-fix`, derive the title from the user's input
goal; a resume-only Build or Fix invocation uses the exact verified run goal. Execute derives its
title from the bounded input. Do not use the slash-command name, project or run identifier, or an
untrusted remote title.
